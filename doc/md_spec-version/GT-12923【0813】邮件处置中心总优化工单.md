...
- 域名使用主机名边界匹配（精确域名及其子域，不会误命中 `evil.com.attacker.test`）；URL 使用规范化后整体精确匹配；附件 MD5 在逗号分隔的多附件哈希中按 token 精确匹配。三类实体均在 DATA 阶段生效：附件 MD5 由 SMTP 热路径的 MIME 解析器对解码后附件内容同步计算，不再等待 sideline 附件安全结果。
