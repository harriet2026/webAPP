...
*说明：因当前验证环境的邮件列表 mock 数据加载异常（与本次改动无关，详见 [H. 需确认事项](#h-需确认事项) Q-002 记录的同一环境限制），本节"变更后"截图直接采用用户在本次对话中提供的真实浏览器实拍；"变更前"（身份验证详情/网络特征模块）的界面已在代码改动时被覆盖，当前环境无法回退重新实拍，已记录为 Q-003，其内容可参照 [1.1 背景](#11-背景) 中的文字描述与本需求提出前的历史截图存档。*
