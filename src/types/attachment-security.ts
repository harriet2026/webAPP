...
/** 风险等级处置动作：隔离 / 审核 / 丢弃，三档均为强制处置，不提供"不设置"
...
/** 超时/引擎不可用后的处置动作，二者可任意组合勾选，均非必选：不勾选任何
 * 动作时默认按原有检测流程继续下一步，不额外处置（不再提供"通知收件人"
 * 选项）。 */
export type SandboxTimeoutActionType = 'recall' | 'notify_admin';

/** 单个风险等级的处置配置：执行动作 + 附加策略两个独立维度，附加策略为
 * “标记”时可再选择标记生效位置。执行动作与附加策略均为必选项（三档风险
 * 等级均不提供"不设置"选项）。 */
export interface SandboxRiskLevelConfig {
  /** 执行动作：隔离 / 审核 / 丢弃（作用于整封邮件的处置，必选）。 */
  action: SandboxRiskAction;
  /** 附加策略：标记 / 丢弃附件（作用于命中风险的附件本身，必选）。 */
...
/** 超时处置配置：超时阈值 + 可组合的超时动作集合。actions 允许为空数组，
 * 不勾选任何动作时表示不额外处置，检测流程默认继续下一步。 */
