...
// 预览里可完整演示（生成 → 复制 DNS TXT → 校验 → 激活 → 删除）。
//
// 数据形状严格对齐 src/lib/api/dkim.ts 的 DkimKey / DkimKeyListResponse /
// VerifyDnsResult，以及 src/types/tenant-domain.ts 的
// TenantDomain。
...
export function mockDkimSigningDomainsFor(tenantId: number): { items: DkimSigningDomain[] } {
  const { items } = mockTenantDomainsFor(tenantId);
  return {
    items: items.map(({ id, tenant_id, domain }) => ({ id, tenant_id, domain })),
  };
}
