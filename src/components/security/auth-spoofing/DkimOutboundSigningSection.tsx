...
import { listDkimKeys, listDkimSigningDomains, type DkimKey } from '@/lib/api/dkim';
 ...
 * DKIM 结果并采取动作；这里管理的是本租户「外发」邮件的 DKIM 签名密钥（服务端
 * 生成/导入私钥、发布 DNS 公钥、校验、激活）。因此本子卡走 DKIM 自己的接口与内存态，
...
function DkimOutboundSigningSectionInner() {
  ...
  const { data: domains, isLoading: domainsLoading } = useQuery({
    ...
    queryFn: () => listDkimSigningDomains(effectiveTenantId as number),
    enabled: canQuery,
  });
