...
import type { DkimKey, DkimKeyListResponse, DkimSigningDomain } from '@/lib/api/dkim';
...
const mockListDkimSigningDomains = vi.fn();
const mockListDkimKeys = vi.fn();
vi.mock('@/lib/api/dkim', () => ({
  listDkimSigningDomains: (id: number) => mockListDkimSigningDomains(id),
...
const domain = (id: number, d: string): DkimSigningDomain => ({
  id,
  tenant_id: 1,
  domain: d,
});
...
describe('DkimOutboundSigningSection', () => {
  ...
  it('renders one row per sending domain with active selector + status', async () => {
    mockListDkimSigningDomains.mockResolvedValue([domain(101, 'a.com'), domain(102, 'b.com')]);
  ...
  it('shows the select-tenant empty state for platform admin without a tenant', async () => {
    ...
    expect(mockListDkimSigningDomains).not.toHaveBeenCalled();
  });

  it('shows the no-domains empty state when the tenant has no sending domains', async () => {
    mockListDkimSigningDomains.mockResolvedValue([]);
