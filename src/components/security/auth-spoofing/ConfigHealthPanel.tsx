...
export function ConfigHealthPanel({ config, onChange }: ConfigHealthPanelProps) {
  ...
  const visible =
    ...
    !(config.spf_observe_mode ?? config.observe_mode) ||
    !(config.dmarc_observe_mode ?? config.observe_mode);

  if (!visible) return null;

  const showSoftfailRow = spfSoftfail?.action === 'discard';
  const showObserveRow =
    (!(config.spf_observe_mode ?? config.observe_mode) && spfFail?.action === 'discard') ||
    (!(config.dmarc_observe_mode ?? config.observe_mode) && dmarcFail?.action === 'discard');
  ...
  const handleEnableObserve = () => {
    onChange({
      ...config,
      ...(!(config.spf_observe_mode ?? config.observe_mode) && spfFail?.action === 'discard' ? { spf_observe_mode: true } : {}),
      ...(!(config.dmarc_observe_mode ?? config.observe_mode) && dmarcFail?.action === 'discard' ? { dmarc_observe_mode: true } : {}),
    });
  };
