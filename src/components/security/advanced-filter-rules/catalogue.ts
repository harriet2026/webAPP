...
export const CONDITIONS: ConditionDef[] = [
  ...
  // GT-12751/方案二裁决：本条件指向后端新增的「图片识别处理状态」字段
  // image_detect_status（success=识别步骤正常跑完，fail=pyhelper 出错/超时截断），
  // 而非二维码解码内容字段 image_qr_code_result（后者是解码文本，语义不匹配
  // success/fail 枚举）。数量诉求由下方 qrCodeCount 承担。
  { key: 'imageQrCodeResult', category: 'attachment', field: 'image_detect_status', panel: 'select' },
  ...
  // 分段阈值（置信度分数落入 [0,1] 区间），取值编码为 JSON 数组，见 serde.encodeIntentEngineList。
