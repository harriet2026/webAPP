...
const ENUM_VALUES: Record<string, string[]> = {
  ...
  // 图片识别处理状态：固定枚举下拉，「成功 / 失败」两态（识别步骤是否正常跑完；
  // fail=pyhelper 出错或超时截断）。标签经 i18n 本地化
  // （v3Conditions.qrResultValues.*），杜绝自由输入产生的脏值。
  image_detect_status: ['success', 'fail'],
  // 信封-头 From 一致性：固定枚举下拉，「一致 / 不一致」两态。token 必须是
  // 布尔字面量 'false'（一致）/'true'（不一致）——后端 envelope_header_mismatch
  // 是 boolean 字段，evaluateBoolCondition 把任何非 "true" 的值当 false，早期
  // 原型的 match/mismatch token 会让「不一致」选项反向命中一致的邮件。标签经
  // i18n 本地化（v3Conditions.envelopeHeaderConsistencyValues.*）。
  envelope_header_mismatch: ['false', 'true'],
};
...
const ENUM_VALUE_I18N_NS: Record<string, string> = {
  image_detect_status: 'qrResultValues',
...
function PanelBody({
  ...
  switch (def?.panel) {
    ...
    default: {
      // 收录进 ENUM_VALUES 的字段优先走带本地化标签的枚举控件——包括 boolean
      // 背书的字段（如信封-头一致性：标签「一致/不一致」比通用「是/否」明确）。
      // boolean 背书时禁用 matchAny：后端 evaluateBoolCondition 只支持 eq/ne，
      // within 会静默恒不命中。
      const enumOpts = leaf.field ? ENUM_VALUES[leaf.field] : undefined;
      if (enumOpts) {
        return <EnumSection leaf={leaf} enumOpts={enumOpts} onChange={onChange} t={t} hasKey={hasKey} allowMulti={fd?.type !== 'boolean'} />;
      }
      if (fd?.type === 'boolean') {
        return (
...
function EnumSection({ leaf, enumOpts, onChange, t, hasKey, allowMulti = true }: { leaf: ConditionLeaf; enumOpts: string[]; onChange: (p: Partial<ConditionLeaf>) => void; t: T; hasKey: (key: string) => boolean; allowMulti?: boolean }) {
  const mode = OPERATOR_TO_MATCH_MODE[leaf.operator] ?? 'equals';
  const multi = allowMulti && mode === 'matchAny';
      ...
      <div className="space-y-1.5">
        ...
        <Select value={mode} onValueChange={(v) => { if (v) onChange({ operator: MATCH_MODE_TO_OPERATOR[v as MatchMode] }); }}>
          ...
          <SelectContent>
            ...
            {allowMulti ? <SelectItem value="matchAny">{t('v3Conditions.matchModes.matchAny')}</SelectItem> : null}
          </SelectContent>
        </Select>
      </div>
      <div className="space-y-1.5">
