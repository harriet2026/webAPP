...
describe('segmentCoverageIssue (GT-12171 D-06 间隙/重叠区分校验)', () => {
  ...
  it('空集 / undefined / 起点终点缺口均为 gap', () => {
    ...
    expect(segmentCoverageIssue([{ min: 0.2, max: 1, action: 'reject' }])).toBe('gap');
    expect(segmentCoverageIssue([{ min: 0, max: 0.8, action: 'reject' }])).toBe('gap');
  });

  it('容差内的边界抖动不算问题', () => {
    const segs: ThresholdSegment[] = [
      { min: 0, max: 0.3005, action: 'accept' },
      { min: 0.3, max: 1, action: 'reject' },
    ];
    expect(segmentCoverageIssue(segs)).toBeNull();
  });
...
describe('fixSegmentCoverage (GT-12171 D-06 智能填充统一修复)', () => {
  it('补齐中间缺口与起点终点', () => {
    const segs: ThresholdSegment[] = [
      { min: 0.1, max: 0.3, action: 'accept' },
      { min: 0.5, max: 0.9, action: 'reject' },
    ];
  ...
  it('被完全吞没的段丢弃', () => {
    const segs: ThresholdSegment[] = [
      { min: 0, max: 1, action: 'accept' },
      { min: 0.2, max: 0.6, action: 'reject' },
    ];
    expect(fixSegmentCoverage(segs)).toEqual([{ min: 0, max: 1, action: 'accept' }]);
  });
