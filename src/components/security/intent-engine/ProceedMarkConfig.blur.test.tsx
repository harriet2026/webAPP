...
function renderCfg(intent: 'subscription' | 'phishing' = 'subscription') {
  ...
  function Harness() {
    const [value, setValue] = useState<IntentMarkConfig>({
      ...
      header_mark: { enabled: true, text: DEFAULT_MARK_TEXT[intent], position: 'prefix' },
    });
    return (
      <ProceedMarkConfig
        value={value}
...
describe('ProceedMarkConfig 清空失焦回填默认文案 (GT-12204)', () => {
  ...
  it('placeholder 同步为该意图默认文案（订阅=[订阅]）', () => {
    ...
    const headerInput = screen.getByTestId('ie-header_mark-text') as HTMLInputElement;
    expect(headerInput.placeholder).toBe(DEFAULT_MARK_TEXT.subscription);
  });
  ...
  it('信头标记同样回填（与保存期 D-11 的兜底范围一致）', async () => {
    const user = userEvent.setup();
    const { onChange } = renderCfg('subscription');

    const input = screen.getByTestId('ie-header_mark-text') as HTMLInputElement;
    await user.clear(input);
    await user.tab();

    const last = onChange.mock.calls.at(-1)?.[0] as IntentMarkConfig;
    expect(last?.header_mark?.text).toBe(DEFAULT_MARK_TEXT.subscription);
  });
  ...
  it('主题标记与信头标记可同时启用（可多选，非互斥）', async () => {
    const { onChange } = renderCfg('subscription');
    // renderCfg 初始状态即两者都 enabled: true，验证组件不会互斥收起任一行的输入框。
    expect(screen.getByTestId('ie-subject_mark-text')).toBeInTheDocument();
    expect(screen.getByTestId('ie-header_mark-text')).toBeInTheDocument();
