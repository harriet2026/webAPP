...
export function DetectionDetailSheet({ open, onOpenChange, detailId }: { open: boolean; onOpenChange: (open: boolean) => void; detailId: string | null }) {
  ...
  const screenshots = investigation?.result?.details?.screenshots ?? [];
  const screenshotsOmittedCount = investigation?.result?.details?.screenshots_omitted_count ?? 0;
  const urlCount = new Set([...findings.map((finding) => finding.url || finding.final_url || ''), ...screenshots.map((capture) => capture.url)]).size;
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent side="right" className="flex flex-col gap-0 p-0 data-[side=right]:w-full data-[side=right]:sm:max-w-2xl" data-testid="phishing-detail-sheet">
    ...
    <div className="flex-1 space-y-5 overflow-y-auto px-6 py-4">{query.isError ? <AssessmentReportView error /> : query.isLoading || !summary ? <div className="flex justify-center py-12"><Loader2 className="size-6 animate-spin text-muted-foreground" /></div> : <>
      ...
      {investigation ? <InvestigationAssessment summary={summary} investigation={investigation} /> : null}
      <AssessmentReportView result={investigation?.result} layout="findings" />
      {urlCount || screenshotsOmittedCount ? <section className="space-y-3 rounded-2xl border border-border/60 bg-muted/20 p-4" data-testid="phishing-url-findings"><h3 className="text-sm font-semibold">{t('table.urlFindingsTitle', { count: urlCount })}</h3><UrlFindingsTable findings={findings} screenshots={screenshots} screenshotsOmittedCount={screenshotsOmittedCount} /></section> : null}
      ...
      <Collapsible open={configOpen} onOpenChange={setConfigOpen}><CollapsibleTrigger data-testid="phishing-detail-config-snapshot" className="flex w-full items-center justify-between rounded-xl border border-border bg-muted/20 px-4 py-2.5 text-sm font-medium"><span>{t('detail.configSnapshot')}</span><ChevronDown className={configOpen ? 'size-4 rotate-180' : 'size-4'} /></CollapsibleTrigger><CollapsibleContent data-testid="phishing-detail-config-snapshot-body" className="pt-2">{query.data?.config_snapshot ? <JsonBlock value={query.data.config_snapshot} /> : <p className="text-sm text-muted-foreground">{t('detail.configPlaceholder')}</p>}</CollapsibleContent></Collapsible>
      <Collapsible open={logOpen} onOpenChange={setLogOpen}><CollapsibleTrigger data-testid="phishing-detail-run-log" className="flex w-full items-center justify-between rounded-xl border border-border bg-muted/20 px-4 py-2.5 text-sm font-medium"><span>{t('detail.runLog')}</span><ChevronDown className={logOpen ? 'size-4 rotate-180' : 'size-4'} /></CollapsibleTrigger><CollapsibleContent data-testid="phishing-detail-run-log-body" className="space-y-2 pt-2"><div className="flex gap-2"><div className="relative min-w-0 flex-1"><Search className="absolute left-2 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input data-testid="phishing-detail-log-search" value={stepSearch} onChange={(event) => setStepSearch(event.target.value)} placeholder={t('detail.searchSteps')} className="h-8 pl-7" /></div><Button data-testid="phishing-detail-export" variant="outline" size="sm" onClick={exportData}><Download className="size-3.5" />{t('detail.exportJson')}</Button></div>{filteredSteps.length ? filteredSteps.map((step, index) => <div key={`${step.name}-${index}`} data-testid={`phishing-detail-step-${index}`} className="rounded-lg border border-border p-2 text-xs"><div className="flex gap-2"><span className="font-medium">{step.name}</span><Badge variant="outline" className="text-xs">{t(`taskStatus.${['pending', 'processing', 'running', 'completed', 'failed', 'partial', 'cancelled'].includes(step.status) ? step.status : 'unknown'}`)}</Badge></div>{step.message ? <p className="mt-1 text-muted-foreground">{step.message}</p> : null}</div>) : <p className="text-sm text-muted-foreground">{t('detail.noSteps')}</p>}</CollapsibleContent></Collapsible>
    </>}</div>
  </SheetContent></Sheet>;
}
