...
import { SANDBOX_RESULT_COLORS } from './colors';

interface AttachmentThreatTypePieCardProps {
  data?: DistItem[];
  isLoading: boolean;
}

export function AttachmentThreatTypePieCard({ data, isLoading }: AttachmentThreatTypePieCardProps) {
  const t = useTranslations('linkAttachmentSecurity');

  const option = useMemo(() => {
    // GT-11996: the backend zero-fills every canonical threat type even when
    // there is no malicious attachment this period, so a bare length check
    ...
    return {
      ...
      series: [{
        ...
        data: data.map((d) => ({
          name: t(`sandboxResult.${d.key}` as Parameters<typeof t>[0]) ?? d.key,
          value: d.count,
          itemStyle: { color: SANDBOX_RESULT_COLORS[d.key] ?? '#8C8C8C' },
        })),
      }],
    };
  }, [data, t]);
