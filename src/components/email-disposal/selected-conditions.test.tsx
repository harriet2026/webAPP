...
describe("SelectedConditions - multi-value quick filter chips (review finding 3/7)", () => {
  it("labels edited conditions as pending until the user applies them", () => {
    render(
      <SelectedConditions
        quick={baseQuick({ sender: "billing@example.com" })}
    ...
    expect(
      screen.getByText(/emailDisposal\.search\.pendingConditions/),
    ).toBeInTheDocument();
  });
