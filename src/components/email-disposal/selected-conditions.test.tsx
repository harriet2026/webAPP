...
describe('SelectedConditions - AI condition chips localization (GT-12368)', () => {
  it('renders AI condition chips with localized field/op/value labels, not raw field keys', () => {
    const aiConditions: AICondition[] = [
      ...
      // 该运行时形态，而不是直接喂 array。
      { field: 'received_at', op: 'between', value: '2026-07-18,2026-07-25', source: 'ai' },
    ...
    expect(screen.getByText(/emailDisposal\.filters\.sendReceiveTime/)).toBeInTheDocument();
    expect(screen.getByText(/emailDisposal\.filters\.emailStatus/)).toBeInTheDocument();
    // display_status 的值走 ENUM_VALUE_MAP -> statuses.delivered，而不是裸的 "delivered"。
    expect(screen.getByText(/emailDisposal\.filters\.statuses\.delivered/)).toBeInTheDocument();
    // 操作符本地化：between -> "~"。
  ...
  it('localizes every value of a comma-flattened multi-value AI enum condition individually (fix round 1)', () => {
    ...
    const aiConditions: AICondition[] = [
      { field: 'display_status', op: 'in', value: 'delivered,rejected', source: 'ai' },
    ];
    render(
      <SelectedConditions
        quick={baseQuick()}
    ...
    expect(
      screen.getByText(/emailDisposal\.filters\.statuses\.delivered,emailDisposal\.filters\.statuses\.rejected/),
    ).toBeInTheDocument();
    // The pre-fix garbled combined-key form must never appear.
    expect(screen.queryByText(/statuses\.delivered,rejected/)).not.toBeInTheDocument();
  });
  ...
  it('falls back to the raw enum value when the i18n key does not exist, instead of showing the dot-joined path', () => {
    ...
    const aiConditions: AICondition[] = [
      { field: 'display_status', op: 'eq', value: 'totally_bogus_status_xyz', source: 'ai' },
    ];
    render(
      <SelectedConditions
        quick={baseQuick()}
    ...
    expect(
      screen.queryByText(/emailDisposal\.filters\.statuses\.totally_bogus_status_xyz/),
  ...
  it('localizes every value of a multi-value `in` enum condition individually', () => {
    const advanced: AdvancedFilter = {
      ...
      groups: [
        {
          operator: 'AND',
          conditions: [{ field: 'action', op: 'in' as never, value: ['quarantine', 'discard'] as never }],
        },
      ],
    };
    render(
    ...
    expect(screen.getByText(/emailDisposal\.filters\.actions\.quarantine,emailDisposal\.filters\.actions\.discard/)).toBeInTheDocument();
  });
  ...
  it('renders the not_in operator with a localized symbol instead of the raw op code', () => {
    const advanced: AdvancedFilter = {
      ...
      groups: [
        {
          operator: 'AND',
          conditions: [{ field: 'action', op: 'not_in' as never, value: ['quarantine'] as never }],
        },
      ],
    };
    render(
