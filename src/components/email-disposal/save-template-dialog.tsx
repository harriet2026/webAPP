...
function buildDefaultName(count: number): string {
  const today = new Date();
  const mm = String(today.getMonth() + 1).padStart(2, "0");
  const dd = String(today.getDate()).padStart(2, "0");
  return `搜索模板 ${today.getFullYear()}-${mm}-${dd}${count > 0 ? ` (${count + 1})` : ""}`;
}
...
export function SaveTemplateDialog({
  ...
  useEffect(() => {
    if (open) {
      setName(initialName ?? buildDefaultName(templateCount));
    }
  }, [open, initialName, templateCount]);
