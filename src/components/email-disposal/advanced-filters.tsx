...
const FIELD_GROUPS: Record<string, FieldEntry[]> = {
  basicInfo: [
    ...
    { key: 'send_hour', i18nKey: 'sendHour', type: 'number', operators: ['eq', 'neq', 'gt', 'lt', 'gte', 'lte', 'between', 'in', 'not_in'] },
    { key: 'storage_size', i18nKey: 'emailSize', type: 'number', operators: ['eq', 'neq', 'gt', 'lt', 'gte', 'lte', 'between', 'in', 'not_in'], unit: 'sizeUnit' },
  ...
  attachment: [
    ...
    { key: 'attachment_total_size', i18nKey: 'attachmentTotalSize', type: 'number', unit: 'sizeUnit' },
    { key: 'attachment_type', i18nKey: 'attachmentType', type: 'text' },
    { key: 'attachment_name', i18nKey: 'attachmentName', type: 'text' },
    { key: 'attachment_md5', i18nKey: 'attachmentMd5', type: 'text', operators: ['eq', 'neq', 'contains', 'not_contains', 'starts_with', 'ends_with', 'regex', 'in', 'not_in'] },
  ],
  securityCheck: [
    {
      key: 'spf_valid', i18nKey: 'spfResult', type: 'enum',
      enumValues: [
    ...
    {
      key: 'dkim_valid', i18nKey: 'dkimResult', type: 'enum',
      enumValues: [
        { value: 'pass', labelKey: 'enumValue.pass' },
        { value: 'fail', labelKey: 'enumValue.fail' },
    ...
    {
      key: 'dmarc_valid', i18nKey: 'dmarcResult', type: 'enum',
      enumValues: [
        { value: 'pass', labelKey: 'enumValue.pass' },
        { value: 'fail', labelKey: 'enumValue.fail' },
        { value: 'none', labelKey: 'enumValue.none' },
    ...
    {
      key: 'similar_domain', i18nKey: 'similarDomain', type: 'enum',
      enumValues: [
    ...
    {
      key: 'display_name_detect', i18nKey: 'displayNameDetect', type: 'enum',
      enumValues: [
    ...
    {
      key: 'virus_scan_result', i18nKey: 'virusScanResult', type: 'enum',
      enumValues: [
    ...
    {
      key: 'intent_label', i18nKey: 'intentEngineResult', type: 'enum',
      enumValues: INTENT_LABEL_OPTIONS,
    },
    {
      key: 'qr_code_result', i18nKey: 'qrCodeResult', type: 'enum',
      enumValues: [
    ...
    {
      key: 'url_result', i18nKey: 'urlResult', type: 'enum',
      enumValues: [
        { value: 'maliciousUrl', labelKey: 'enumValue.maliciousUrl' },
        { value: 'suspicious', labelKey: 'enumValue.suspicious' },
        { value: 'normal', labelKey: 'enumValue.normal' },
    ...
    {
      key: 'rbl_result', i18nKey: 'rblResult', type: 'enum',
      enumValues: [
        { value: 'triggered', labelKey: 'enumValue.triggered' },
        { value: 'notTriggered', labelKey: 'enumValue.notTriggered' },
      ],
    ...
    {
      key: 'threat_level', i18nKey: 'threatLevel', type: 'enum',
      enumValues: [
...
function ConditionValueInput({ cond, fieldDef, onChange }: ConditionValueInputProps) {
    ...
    return (
      <div className="flex flex-1 items-center gap-1">
        <Input
          ...
          value={String(from ?? '')}
          onChange={(e) => onChange({ value: [e.target.value, String(to ?? '')] })}
        />
        <Input
          className="h-8 flex-1 text-xs"
          type="number"
          ...
          value={String(to ?? '')}
          onChange={(e) => onChange({ value: [String(from ?? ''), e.target.value] })}
        />
        {unitSuffix}
      </div>
    );
  ...
  if (type === 'boolean') {
    ...
    const boolLabel = boolVal === 'true' ? t('enumValue.booleanTrue') : boolVal === 'false' ? t('enumValue.booleanFalse') : '';
    return (
      <Select value={boolVal} onValueChange={(v) => onChange({ value: v ?? '' })}>
  ...
  if (type === 'enum' && fieldDef?.enumValues) {
    ...
    return (
      <Select value={strVal} onValueChange={(v) => onChange({ value: v ?? '' })}>
        <SelectTrigger className="h-8 flex-1 text-xs">
          <SelectValue>{matched ? t(matched.labelKey) : t('placeholder.select')}</SelectValue>
        </SelectTrigger>
        <SelectContent>
...
export function AdvancedFilters({ value, onChange }: AdvancedFiltersProps) {
  ...
  const addGroup = () => {
    onChange({ ...value, groups: [...value.groups, { operator: 'AND', conditions: [] }] });
  };
  ...
  const setGroupOperator = (gi: number, op: 'AND' | 'OR') => {
    const groups = value.groups.map((g, i) => (i === gi ? { ...g, operator: op } : g));
    onChange({ ...value, groups });
  };

  const addCondition = (gi: number) => {
    const groups = value.groups.map((g, i) =>
      i === gi ? { ...g, conditions: [...g.conditions, { field: '', op: 'eq' as const }] } : g,
    );
    onChange({ ...value, groups });
  };
  ...
  const handleFieldChange = (gi: number, ci: number, fieldKey: string) => {
    ...
    updateCondition(gi, ci, { field: fieldKey, op: newOp as FilterCondition['op'], value: undefined });
  };

  return (
    <Collapsible open={open} onOpenChange={setOpen} data-testid="disposal-advanced-filter">
      <CollapsibleTrigger
      ...
      >
        <ChevronDown className={`h-4 w-4 transition-transform duration-[240ms] ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none ${open ? '' : '-rotate-90'}`} />
        {t('advanced')}
        <span className="text-muted-foreground">({t('preciseFields')})</span>
        <Info className="h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
        {value.groups.some((g) => g.conditions.length > 0) && (
          <span className="ml-1 rounded-full bg-primary/10 px-1.5 py-0.5 text-xs text-primary">
            {value.groups.reduce((sum, g) => sum + g.conditions.length, 0)}
          </span>
        )}
      </CollapsibleTrigger>
      <CollapsibleContent className="mt-3 space-y-3">
        {value.groups.length > 1 && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <Select value={value.operator} onValueChange={(operator) => onChange({ ...value, operator: operator as 'AND' | 'OR' })}>
              <SelectTrigger data-testid="disposal-advanced-top-operator" className="h-7 w-24 text-xs"><SelectValue /></SelectTrigger>
        ...
        {value.groups.map((group, gi) => (
          <div key={gi} data-testid={`disposal-advanced-group-${gi}`} className="rounded-lg border p-3 space-y-2">
            <div className="flex items-center justify-between gap-2">
              <Select value={group.operator} onValueChange={(v) => setGroupOperator(gi, v as 'AND' | 'OR')}>
            ...
            {group.conditions.map((cond, ci) => {
              ...
              return (
                <div key={ci} data-testid={`disposal-advanced-condition-${gi}-${ci}`} className="flex items-center gap-2">
                  <Select value={cond.field ?? ''} onValueChange={(v) => handleFieldChange(gi, ci, v ?? '')}>
                    <SelectTrigger className="h-8 min-w-[12rem] shrink-0 text-xs">
                    ...
                    <SelectContent>
                      {Object.entries(FIELD_GROUPS).map(([groupKey, fields]) => (
                        <div key={groupKey}>
                          ...
                          {fields.map((f) => (
                            <SelectItem key={f.key} value={f.key}>{t(f.i18nKey)}</SelectItem>
                          ))}
                        </div>
                      ))}
                    </SelectContent>
                  </Select>
                  <Select value={cond.op} onValueChange={(v) => updateCondition(gi, ci, { op: v as FilterCondition['op'], value: undefined })}>
                    <SelectTrigger className="h-8 min-w-[9rem] shrink-0 text-xs">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {allowedOps.map((op) => (
            ...
            <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => addCondition(gi)}>
              <Plus className="mr-1 h-3 w-3" />
              {t('addCondition')}
            </Button>
          </div>
        ))}
        <Button data-testid="disposal-advanced-add-group" variant="outline" size="sm" className="h-7 text-xs" onClick={addGroup}>
          <Plus className="mr-1 h-3 w-3" />
          {t('addGroup')}
        </Button>
      </CollapsibleContent>
