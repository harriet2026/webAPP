...
vi.mock("@/components/shared/page-filters", () => ({
  PageFilters: ({ children }: { children: React.ReactNode }) =>
    createElement("div", null, children),
}));
...
describe("EmailDisposalCenterPage tenant-switch reset (review TEST-3)", () => {
  ...
  it("applies a page-local platform scope only after Search is clicked", async () => {
    ...
    // 筛选区采用渐进披露；平台租户选择器也随结构化筛选一起按需展开。
    fireEvent.click(getByTestId("disposal-filters-toggle"));
  ...
  it("does not re-fetch while a text filter is being edited", async () => {
    useTenantMock.mockReturnValue({
      effectiveTenantId: null,
      selectedTenantId: null,
    });
