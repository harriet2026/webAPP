...
test.describe('Email Disposal Center', () => {
  ...
  test('quick filter section visible', async ({ authenticatedPage }) => {
    // GT-12423: 高级筛选默认展开（helper 幂等保证展开态）。
  ...
  test('advanced filters toggle', async ({ authenticatedPage }) => {
    // 「更多筛选条件」(AdvancedFilters) 在筛选区内（GT-12423 起默认展开）。
    await ensureFiltersExpanded(authenticatedPage);
...
// GT-12423: 高级筛选默认展开（html_spec index「按 demo（默认展开）落地」）；
...
test.describe('disposal responsive layout (GT-12423)', () => {
  ...
  test('filters are expanded by default', async ({ authenticatedPage: page }) => {
    await page.goto('/zh/email-disposal/center');
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('disposal-quick-filters')).toBeVisible({ timeout: 15000 });
  });

  test('1024px keeps first four filters on one row and table scrolls horizontally', async ({ authenticatedPage: page }) => {
    await page.setViewportSize({ width: 1024, height: 1080 });
    await page.goto('/zh/email-disposal/center');
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('disposal-quick-filters')).toBeVisible({ timeout: 15000 });
    const ids = ['disposal-date-range', 'disposal-direction-filter', 'disposal-sender-filter', 'disposal-recipient-filter'];
    const ys: number[] = [];
    for (const id of ids) {
...
test.describe('disposal action dialogs (GT-12420/GT-12419)', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    await request.post(
      (process.env.INTERNAL_API_BASE_URL || 'https://localhost:18081') + '/internal/mail-logs/ingest',
      {
        data: [{
  ...
  async function filterStatus(page: import('@playwright/test').Page, label: string) {
  ...
  async function tokenColor(page: import('@playwright/test').Page, cssVar: string) {
  ...
  async function openAndMeasure(
    ...
    if (confirmColorVar) {
      ...
      const confirm = dialog
        ...
        .last();
      const actual = await confirm.evaluate((el) => getComputedStyle(el).backgroundColor);
      expect(actual).toBe(await tokenColor(page, confirmColorVar));
    }
    return dialog;
  }

  test('delete dialog is 448px wide with red confirm', async ({ authenticatedPage: page }) => {
    await page.goto('/zh/email-disposal/center');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1500);
    const dialog = await openAndMeasure(page, 'disposal-batch-delete', 'disposal-delete-dialog', '--color-red-500');
    await dialog.getByRole('button', { name: '取消', exact: true }).click();
  });

  test('recall dialog is 448px wide with orange confirm', async ({ authenticatedPage: page }) => {
    await page.goto('/zh/email-disposal/center');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1500);
    await filterStatus(page, '投递成功');
    const dialog = await openAndMeasure(page, 'disposal-batch-recall', 'disposal-recall-dialog', '--color-orange-500');
    await dialog.getByRole('button', { name: '取消', exact: true }).click();
  });

  test('release dialog is 448px wide with green confirm', async ({ authenticatedPage: page }) => {
    await page.goto('/zh/email-disposal/center');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1500);
    await filterStatus(page, '隔离中');
    const dialog = await openAndMeasure(page, 'disposal-batch-release', 'disposal-release-dialog', '--color-green-500');
    await dialog.getByRole('button', { name: '取消', exact: true }).click();
  });
});
